...
/*!50001 VIEW `bb` AS select `bowls`.`petAge` AS `BowlPetAge`,`beds`.`petAge` AS `petAge`,count(0) AS `count` from ((`prods` left join `beds` on((`prods`.`id` = `beds`.`prodId`))) left join `bowls` on((`prods`.`id` = `bowls`.`prodId`))) where ((`prods`.`petTypeId` = 1) and (`prods`.`type` = 'Содержание и уход')) group by `bowls`.`petAge`,`beds`.`petAge` */;
